...
This external module allows you to encrypt certain fields in surveys, using mixed encryption with openssl (RSA + AES-256 in CBC mode), thus making it impossible for hackers or malicious system administrators to read these fields (Only a public key is stored on the server). Servers do get hacked and health care systems have recently increasingly been targets.
...
2. The encrypted text is a json string however, when you open the form in REDCap, encryped fields will be displayed as "Encrypted field (cannot be modified)" and it will be readonly. The logs though will display the json string.

## Limitations

1. The encryption currently only works in surveys.
2. There is currently no decryption in REDCap which means also that the responses cannot be edited. Therefore you cannot have encryption and the save and return option enabled in a survey.
3. The private key is not stored on the server. This means that you cannot decrypt the data if you loose the private key, so make sure you keep copies of the private key and/or copies of the decrypted data in safe places.
4. Mixed encryption at the field level largely increases the amount of space required for storage, therefore this solution would not be recommended for a large number of fields and a large number of records. A typical use case is when you have PHIs  in a public survey, e.g. consent/registration form, and encypt those, but not the analytical reasearch data in other forms. In thi